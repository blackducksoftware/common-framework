...
public class DataTableTest {
	...
	public void test() throws Exception {
		...
		fields.add(new FieldDef("publishedDate", FieldType.DATE, "Published Date", "yyyy-MM-dd HH:mm:ss.0"));
		...
		assertEquals("applicationName test value", record.getStringFieldValue("applicationName"));
		assertEquals("applicationVersion test value", record.getStringFieldValue("applicationVersion"));
		assertEquals("componentName test value", record.getStringFieldValue("componentName"));
		assertEquals("componentVersion test value", record.getStringFieldValue("componentVersion"));
		assertEquals("cveName test value", record.getStringFieldValue("cveName"));
		assertEquals("vulnerabilityDescription test value", record.getStringFieldValue("vulnerabilityDescription"));
		assertEquals("http://www.google.com", record.getHyperlinkFieldValue("link").getHyperlinkText());
		assertEquals("hyperlink display text", record.getHyperlinkFieldValue("link").getDisplayText());
		assertEquals("2000-01-01 00:00:00.0", record.getDateFieldValue("publishedDate"));
		assertEquals("2000-01-01 00:00:00.0", record.getStringFieldValue("publishedDate"));
		assertFalse(dataSetIter.hasNext());
		...
		assertEquals(expectedDate, record.getDateFieldValueAsDate("publishedDate"));
		
		assertTrue(dataSetIter.hasNext());
		record = dataSetIter.next();
		assertEquals("applicationName test value2", record.getStringFieldValue("applicationName"));
		assertEquals("applicationVersion test value2", record.getStringFieldValue("applicationVersion"));
		assertEquals("componentName test value2", record.getStringFieldValue("componentName"));
		assertEquals("componentVersion test value2", record.getStringFieldValue("componentVersion"));
		assertEquals("cveName test value2", record.getStringFieldValue("cveName"));
		assertEquals("vulnerabilityDescription test value2", record.getStringFieldValue("vulnerabilityDescription"));
		assertEquals("http://www.blackducksoftware.com", record.getHyperlinkFieldValue("link").getHyperlinkText());
		assertEquals("hyperlink display text2", record.getHyperlinkFieldValue("link").getDisplayText());
		assertEquals("2001-01-01 00:00:00.0", record.getDateFieldValue("publishedDate"));
		assertEquals("2001-01-01 00:00:00.0", record.getStringFieldValue("publishedDate"));
	...
	public void testTruncation() throws Exception {
		...
		// Create the RecordDef
		List<FieldDef> fields = new ArrayList<FieldDef>();
		fields.add(new FieldDef("sealId", FieldType.STRING, "SEAL ID", 5)); // truncate values to 5 chars
		...
		// Add a record to the dataset
		Record record = new Record(recordDef);
		record.setFieldValue("sealId", "a              b"); // "b" should get truncated off; spaces should get trimmed
		dataSet.add(record);
		assertEquals(1, dataSet.size());
