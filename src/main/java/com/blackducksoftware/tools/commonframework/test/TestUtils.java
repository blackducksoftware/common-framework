...
public class TestUtils {
	...
	File tempFile = File.createTempFile("unitTest" + ++tempFileIndex,
		".xlsx");
     ...
     * @throws Exception
     */
    public static void checkReport(String expectedReportFilename,
	    String actualReportFilename, boolean firstDataRowOnly,
	    boolean beFlexible) throws Exception {

	// System.out.println("Expected: " + expectedReportFilename +
	// "; Actual: " + actualReportFilename);
	Workbook expectedWorkbook = WorkbookFactory.create(new File(
		expectedReportFilename));
	Workbook actualWorkbook = WorkbookFactory.create(new File(
		actualReportFilename));

	for (int sheetIndex = 0; sheetIndex < expectedWorkbook
		...
		while (expectedCellIter.hasNext()) {
		    compareCells(expectedCellIter, actualCellIter, colIndex++,
			    beFlexible);
		}
    ...
    private static void compareCells(Iterator<Cell> expectedCellIter,
	    Iterator<Cell> actualCellIter, int colIndex, boolean beFlexible) {
    ...
    private static boolean match(String expectedValue, String actualValue,
	    boolean beFlexible) {
	// System.out.println("Comparing [" + actualValue + "] to [" +
	// expectedValue + "]. beFlexible=" + beFlexible);
	...
	if ((s.contains("January")) || (s.contains("February"))
		|| (s.contains("March")) || (s.contains("April"))
		|| (s.contains("May")) || (s.contains("June"))
		|| (s.contains("July")) || (s.contains("August"))
		|| (s.contains("September")) || (s.contains("October"))
		|| (s.contains("November")) || (s.contains("December"))) {
	    return true;
	}
	return false;
    }
