...
import soleng.framework.standard.protex.report.template.TemplateReader;
...
public class TemplateReaderTest extends TemplateTestConstants
{
	...
	public void testTemplateSheet() throws Exception
	{
		...
		Map<String, TemplateColumn> columnsFromSheetTwo = sheetTwo.getColumnMap();
	...
	public void testTemplateSheetStylesUniqueness() throws Exception
	{
		...
		TemplateColumn columnB = columns.get(COLUMN_B);
		TemplateColumn columnC = columns.get(COLUMN_C);
	...
	/**
	 * Tests user specified column/name mappings
	 * We provide a ConfigurationManager to the Template Reader.
	 *
	 * @throws Exception the exception
	 */
	@Test
	...
	{
		exception.expect(Exception.class);
		exception.expectMessage("The following style position is not defined: " + 0);
		tr.copyTemplateIntoFile(BAD_TEMPLATE_FORMAT, workingFile);		
	}
	
	/**
	 * Test the "no copy" usage that SCM Connector uses.
	 * Generate a workbook from a template file, and populate the template map.
	 * @throws Exception
	 */
	@Test
	public void testNoCopyUsage() throws Exception {
		...
		Workbook wb = TemplateReader.generateWorkBookFromFile(new File("src/test/resources/test_excel_template.xlsx"));
		...
		Assert.assertEquals((double)1, wb.getSheetAt(0).getRow(1).getCell(2).getNumericCellValue(), (double)0.01);
	...
	/**
	 * Convenient test method that makes sure each object contains appropriate values.
	 *
	 * @param columns the columns
	 * @param expectedName the expected name
	 * @param expectedPos the expected pos
	 */
