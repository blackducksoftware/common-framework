...
/**
 * The standard login screen, with a logo, server/username/password fields, and a login button.
 * @author sbillings
 *
 */
public class StandardLoginScreen {
	...
	private static final boolean debugMode = false; // in debug mode, version shown is of this class, not app
	 ...
	 *
	 * @param clientBrowserElementId Location in HTML of login screen
	 * @param clientLoginService An object we can use to make the login() RPC call
	 * @param clientLoginHandler The object we should notify upon successful login
	 * @param loginInfo The object to populate with login details and pass back and forth to server
	 * @param title the title
	 * @param appVersionString A version string for the application
	 * @param serverCookie Cookie: server
	 * @param userCookie Cookie: user
	 * @param passwordCookie Cookie: password
	 * @param logoPath Path to the logo to display
	 */
	public StandardLoginScreen(String clientBrowserElementId,
			LoginService clientLoginService,
			LoginHandler clientLoginHandler,
			AuthenticationUser loginInfo,
			String title, 
			String appVersionString, 
			String serverCookie, String userCookie, String passwordCookie,
			String logoPath) {
	 ...
	 * @param appVersionString the app version string
	 * @param title the title
	 */
	...
	private TextBox buildServerBox() {
		...
		String serverValue = debugMode ? "http://seneca.blackducksoftware.com" : ".blackducksoftware.com";
	...
	private TextBox buildUserNameBox() {
		...
		String userNameValue = debugMode ? "unitTester@blackducksoftware.com" : "@blackducksoftware.com";
	...
	private class LoginRequestHandler implements ClickHandler, KeyUpHandler, KeyPressHandler {
		...
		/**
		 * Send the login request to the server and wait for a
		 * response.
		 */
		public void login() {
			GWT.log("Login handler login() called");
			
			// Load the cookies, but this time memorize (provided checkbox is marked)
			loadCookies(true);
			...
			// The client-provided service actually makes the RPC call, but we get the callback
	 ...
	 *
	 */
	private class LoginCompleteCallback implements AsyncCallback<AuthenticationUser> {
		...
		public void onSuccess(AuthenticationUser user) 
		{
			...
			else
			{
				...
				errorLabel.setVisible(true);
				errorLabel.setText("Authentication failure: " + user.getErrorMessage());
				loginButton.setEnabled(true);
			}
		}
	}
