...
public class TrackableRunnerTest {
	...
	public void testUnresponsiveWorker() throws Throwable {
		...
		for (tryCount = 1; tryCount <= 100; tryCount++) {
			...
			percentCompleted = workerRunner.getProgress();
				...
				exceptionThrown = e;
				assertEquals("No response from worker in " + MAX_TOLERABLE_SILENCE + " tries.", e.getMessage());
				break;
			}
			if (percentCompleted >= 100) {
				break;
		...
		assertEquals(MAX_TOLERABLE_SILENCE+1, tryCount); // if we didn't detect worker's death, we've failed
	}
	...
	public void testSuicidalWorker() throws Throwable {
		...
		try {
			...
			assertEquals("java.lang.NullPointerException: SuicidalWorkerMock committing suicide.", e.getMessage());
		}
	}
