...
import soleng.framework.core.config.testbeans.TestConfigurationManagerBean;
...
{
    public static final Logger log = LoggerFactory.getLogger(EmailNotifierImplTest.class);
    ...
    {
    	fullLocationOfEmailConfig = ClassLoader.getSystemResource(emailProp).getFile();
    	fullLocationOfEmailTemplateFile = ClassLoader.getSystemResource(notificationFile).getFile();
    	String fullLocationAuth = ClassLoader.getSystemResource(emailAuthProp).getFile();
	try
	{
    ...
    /**
     * Make sure we can generate an email like the SCM Connector does, in advance of
     * pointing SCM Connector to Common Framework for EmailNotifierImpl.
     * 
     * @throws Exception
     */
    @Test
	public void testScmConnectorLikeEmail() throws Exception 
	{
		...
		props.setProperty("protex.user.name", "unitTester@blackducksoftware.com");
		props.setProperty("protex.password", "blackduck");
		
		ConfigurationManager config = new TestProtexConfigurationManager(fullLocationOfEmailConfig);
		
		CFEmailNotifier notif = new CFEmailNotifier(config);
		EmailContentMap values = notif.configureContentMap(fullLocationOfEmailTemplateFile);	
		...
		values.put("scanType", "Initial Baseline"); // ***** TODO: the "info." prefix breaks this for some reason
		...
		// This realistically only tests whether the keys are being mapped, not the actual sending.
		// No rules
