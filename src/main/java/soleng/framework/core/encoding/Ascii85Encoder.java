...
/**
 * Methods for Ascii85 (aka Base85) encoding and decoding.
 * Data to be encoded must be a byte array whose length is a multiple of 4.
 * The encoded Ascii85 data will consist of printable ASCII characters, and will have
 * a length 25% longer than the original data.
 * 
 * Ascii85 encode/decode algorithms adapted from https://pdfbox.apache.org/ org.apache.pdfbox.io package.
 * I've done a significant amount of refactoring to make the code more readable, but it could use more.
 */
public class Ascii85Encoder {
    ...
    /**
     * Ascii85-encode the given (binary) data.
     * Data to be encoded must be a byte array whose length is a multiple of 4.
     * Ascii85 encoding produces five ASCII printable characters from
     * each chunk of four bytes of binary data.
     * 
	...
	public static byte[] encode(byte[] bytesToBeEncoded) {
	    ...
	    for (int i=0; i < bytesToBeEncoded.length; i++) {
	    	...
	    	// if we have filled inputChunk buffer, encode its contents into outputChunk
	        if (indataIndex == 4) {
	...
	/**
     * This will transform the given four ascii bytes to Ascii85 encoding (5 ascii chars).
     * This method is copied nearly as-is from the transformASCII85 method in apache pdfbox.
     */
	...
	public static byte[] decode(byte[] bytesToBeDecoded) throws DecoderException {
		byte[] chunkOfFiveCharsToDecode = new byte[5]; // Chunk of 5 chars to decode (into 4 binary bytes)
		byte[] chunkOfFourDecodedBytes = new byte[4]; // Chunk of 4 decoded bytes
		...
		for (int bytesToBeDecodedIndex=0; bytesToBeDecodedIndex < bytesToBeDecoded.length; ) {
            ...
            {
                chunkOfFiveCharsToDecode[0] = byteToBeDecoded; // may be EOF here....
